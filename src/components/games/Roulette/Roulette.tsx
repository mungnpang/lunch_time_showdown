...
export default function Roulette() {
  ...
  const spin = () => {
    ...
    const randomDegree = extraSpins * 360 + Math.floor(Math.random() * 360);
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < options.length; i++) {
        const centerAngle = (i + 0.5) * itemAngle;
        let currentPos = (centerAngle + finalRotation) % 360;
        
        let distance = Math.abs(currentPos - 270);
        ...
        <div className={styles.wheelWrapper}>
          <div className={styles.pointer}>
            <svg viewBox="0 0 30 40" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path 
                d="M5 2h20c3 0 4 1 3 4L17 36c-1 3-3 3-4 0L2 6c-1-3 0-4 3-4z" 
                fill="#ffffff" 
              />
              <path 
                d="M7 4h16c2 0 3 1 2 3L16 33c-1 2-2 2-2 0L5 7c-1-2 0-3 2-3z" 
                fill="#ff5252" 
              />
            </svg>
          </div>
        ...
        <div className={styles.optionsList}>
          {options.map((opt, i) => (
            <div key={i} className={styles.optionChip} onClick={() => removeOption(i)}>
              <div className={styles.colorDot} style={{ backgroundColor: COLORS[i % COLORS.length] }}></div>
              <span>{opt}</span>
